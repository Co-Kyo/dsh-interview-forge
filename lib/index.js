...
import { ForgeGateway } from './forge-gateway.js'
...
function createStore() {
  ...
  store.latestEntry = () => (store.order.length === 0 ? undefined : store.sessions.get(store.order[store.order.length - 1]))
  store.findEntry = (sid) => {
    if (!sid) return store.latestEntry()
    if (store.sessions.has(sid)) return store.sessions.get(sid)
    return store.entryForOwner(sid)
  }
  store.entryForOwner = (ownerSid) => {
